#!/bin/bash
#PBS -l select=1:ncpus=1:mem=250Gb:scratch_local=250gb
...
cp $DATADIR/kmerGO_21mer/kmer_matrix/son_matrix_*.txt $SCRATCHDIR
cp $DATADIR/21mer_femalespecific.txt $SCRATCHDIR
cp $DATADIR/21mer_malespecific.txt $SCRATCHDIR || exit 1
...
cp -r ${SCRATCHDIR}/* $DATADIR/sex-specific_21mer 
rm ${SCRATCHDIR}/*



cp $DATADIR/kmerGO_55mer/kmer_matrix/son_matrix_*.txt $SCRATCHDIR
cp $DATADIR/55mer_femspecific.txt $SCRATCHDIR
cp $DATADIR/55mer_malespecific.txt $SCRATCHDIR
...
cp -r ${SCRATCHDIR}/* $DATADIR/sex-specific_55mer || export CLEAN_SCRATCH=false
